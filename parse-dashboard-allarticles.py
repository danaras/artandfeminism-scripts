...
dashboardCSV = 'artfeminism_2018-articles-2018-09-22.csv'
firstline = True
count = 0
...
with open(dashboardCSV, 'r') as f:
	...
	for line in reader:
		...
		if "wikidata" not in wiki:
			...
			print title
			ns = int(info[1])
			new = str(info[6]).lower().encode('utf8')
			deleted = str(info[7]).lower().encode('utf8')
			if (ns is 0) and ("true" in new) and ("false" in deleted):
				csvWriter1.writerow([language, title])
			elif (ns is 0) and ("false" in new) and ("false" in deleted):
				csvWriter2.writerow([language, title])
			elif (ns is 118) and ("false" in deleted):
				csvWriter3.writerow([language, title, info[3], new, deleted])
			elif ((ns is 118) or (ns is 0))and ("true" in deleted):
				csvWriter4.writerow([language, title, info[3], info[10], new, deleted])
			else:
				csvWriter5.writerow([language, title, info[1], info[3], info[10], new, deleted])
